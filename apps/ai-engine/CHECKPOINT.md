...
- 2026-06-05 — Added macOS-usable process-tree profiling and validated the Kim vocal-isolation model cache.
  ...
  - Validation: `./.venv/bin/python -m py_compile src/utils/hardware_profiler.py src/scripts/benchmark_suite.py src/utils/vocal_isolator.py`; smoke run wrote `outputs/profiles-smoke/profile_20260605_143952_jobsmoke.txt`; `./.venv/bin/python - <<'PY' from src.utils.vocal_isolator import VocalIsolator; VocalIsolator()._get_separator(); PY` downloaded and loaded `temp/models/Kim_Vocal_2.onnx` (~`64 MB`); `./scripts/run-e2e-youtube-pipeline.sh --case-id english_-moW9jvvMr4 --target-language vi --output-dir outputs/e2e-benchmarks/runs/mac-english-vi-profiled` completed end to end and produced `outputs/profiles/profile_20260605_144415_job9.txt` with `DEVICE=cpu`, average process CPU `558.1%`, peak process CPU `814.7%`, average process RSS `7.55 GB`, peak process RSS `9.45 GB`, ASR total `152.401s`, and consumer NMT `133.433s`.
  - Follow-up: For apples-to-apples comparison with the Windows/NVIDIA machine, run the same profiled E2E on the PC after syncing these profiler changes. The Mac data already indicates the current bottleneck is CPU-bound ASR/NMT, not MinIO or upload time.

- 2026-06-05 — Completed the translated macOS app-path E2E and hardened the local runner against stale queue consumers.
  - Status: Working
  - Changed: Updated `src/minio_client.py` so processed-artifact uploads retry once with a rebuilt MinIO client when the server returns `RequestTimeTooSkewed`, and log both local UTC time and the server `Date` header for future debugging. Updated `scripts/run-e2e-youtube-pipeline.sh` so it also kills stale `dist/src/worker.js` and `python -m src.main` processes before starting a new run, instead of only freeing port `3000`.
  - Why: The first translated macOS reruns were failing for two different non-model reasons: a stale AI-engine worker from an older session could still consume BullMQ jobs and report misleading CUDA-related failures, and a clean translated run had previously died later on a MinIO chunk upload with `RequestTimeTooSkewed`. The runner now isolates each E2E session cleanly, and the AI engine keeps a recovery guard for the late-upload skew case.
  - Validation: `bash -n scripts/run-e2e-youtube-pipeline.sh`; `./.venv/bin/python -m py_compile src/minio_client.py`; `./scripts/run-e2e-youtube-pipeline.sh --case-id english_-moW9jvvMr4 --target-language vi --output-dir outputs/e2e-benchmarks/runs/mac-english-vi-keep3` completed end to end with `completedStatus=COMPLETED`, `source_lang=en`, `target_lang=vi`, `63` final segments, average WER `0.01`, average wall-clock latency `207.686s`, first chunk `90.377s`, and first translated batch `90.377s`.
  - Follow-up: The validated Mac path now covers the English default ASR route plus NLLB translation. Chinese-route caches are present locally (`SenseVoiceSmall`, `paraformer_zh`, Qwen forced aligner), but Chinese-source live validation should be run separately before treating full zh-path parity as verified on this laptop.

- 2026-06-05 — Hardened macOS model-download bootstrap and revalidated the live English app path on Apple Silicon.
  - Status: Partial
  - Changed: Updated `src/config.py` so `.env`-only Hugging Face settings now propagate into the real process environment for downstream clients, including `HF_TOKEN`, `HF_HUB_DISABLE_XET`, and `HF_HUB_DOWNLOAD_TIMEOUT`. Updated `src/scripts/convert_nllb.py` so `python -m src.scripts.convert_nllb` resolves `ct2-transformers-converter` from the active `.venv/bin/` directory instead of relying on an activated shell PATH. Updated macOS env defaults in `.env` and `.env.mac` to prefer the Hugging Face FunASR mirror (`FUNASR_MODEL_HUB=hf`) and disable the Xet transfer backend on this Mac setup.
  - Why: The MacBook bootstrap had three concrete setup failures: Hugging Face credentials stored in `.env` were invisible to library downloads, the NLLB conversion helper failed when invoked through `./.venv/bin/python`, and large Hub downloads were stalling/resetting under the `hf-xet` transfer backend. Those issues blocked clean model acquisition even before runtime validation.
  - Validation: `./.venv/bin/python -m py_compile src/config.py src/scripts/convert_nllb.py`; `./scripts/run-e2e-youtube-pipeline.sh --case-id english_-moW9jvvMr4 --target-language en --output-dir outputs/e2e-benchmarks/runs/mac-english-en` completed end to end with `completedStatus=COMPLETED`, `source_lang=en`, `target_lang=en`, `63` final segments, average WER `0.01`, and wall-clock latency `180.464s` from the real backend -> worker -> AI-engine path.
  - Follow-up: Let the long-running `facebook/nllb-200-3.3B` CT2 conversion finish under the new non-Xet settings, then rerun the same E2E harness with `--target-language vi` to validate live translation on macOS. After that, re-check the Chinese route downloads (`sensevoice_small`, `paraformer_zh`, `whisper_full`, `Qwen3-ForcedAligner-0.6B`) if full Chinese-path parity is still required on this laptop.
